...
{
  ros::init(argc, argv, "robot"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
  ...
  ROS_INFO("robot node has turned on "); //Prompt that the robot node has been created //提示已创建robot节点 
  return 0;  
} 
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("usart_port_name",  usart_port_name,  "/dev/controller"); //Fixed serial port number //固定串口号
  ...
  {
    ROS_INFO_STREAM("robot serial port opened"); //Serial port opened successfully //串口开启成功提示
  }
}
/**************************************
Date: January 28, 2021
