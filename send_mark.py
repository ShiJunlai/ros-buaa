...
def status_sub(msg):
    global status,markerArray,count,index
    status = msg.data
    if status == 0:
        markerArray = MarkerArray()
        count = 0
        index = 0
    mark_pub.publish(markerArray) #发布markerArray，rviz订阅并进行可视

        
def send_mark():
    global markerArray, count, index, try_again, mark_pub, goal_pub,status
    ...
    model_sub = rospy.Subscriber('/status',Int32,status_sub)#单点或者多点模式
